...
<template>
    <Dialog :open="modelValue" @update:open="emit('update:modelValue', $event)">
        <DialogContent class="max-w-md">
            ...
            <div class="space-y-4 py-2">
                ...
                <div v-if="isUploading" class="space-y-2">
                    ...
                    <p class="text-xs text-muted-foreground text-center">
                        {{ progress < 100 ? `Subiendo... ${progress}%` : 'Procesando...' }} </p>
                </div>

                <!-- Mensaje de éxito o error -->
                <transition name="fade">
                    <div v-if="message" class="flex flex-col items-center mt-3">
                        <component :is="message.startsWith('✅') ? CheckCircle : XCircle" class="h-6 w-6 mb-1"
                            :class="message.startsWith('✅') ? 'text-green-600' : 'text-red-600'" />
                        <p class="text-sm font-medium text-center" :class="{
            ...
            <DialogFooter>
                <Button variant="secondary" @click="emit('update:modelValue', false)" :disabled="isUploading">
                    Cancelar
                </Button>
